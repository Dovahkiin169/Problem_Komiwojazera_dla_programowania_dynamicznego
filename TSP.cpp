...
void TSP::TabuMove(int i, int j, int length, int** tabu)//zamienamy długość dwóch sąsiedncih elementów
 {
	tabu[i][j] = length;
	tabu[j][i] = length;
 }
		...
		for(int j = 0; j<this->Size; j++)
		 {
			...
			if (nearestDistance > distances[Route[i-1]][j])
			 {
				nearestIndex = j;
				nearestDistance = distances[Route[i-1]][j];
			 }
		 }
		Visited[nearestIndex] = true;//Ustawiamy że już byłi w tym wierszcholku
		Route[i] =  nearestIndex;
	 }
	return Route;
 }
	...
	while(++Counter < Iterations)//Wykonujemy tę algotyrm Iterations raz
	 {
		...
		for (int i = 0; i < Size - 1; i++)
		{
			for (int j = i + 1; j < Size; j++)
			{
				...
				{
					this->TabuMove(OptimalRoute[i], OptimalRoute[j], TabuLen, ListTabu);
		...
		if (OptimalLocalRouteCost < OptimalRouteCost)//Jezeli nowy coszt tej iteracji jest mniejszy od poprzedniego wiadomego poprzedniego tej iteracji
		 {
			OptimalRouteCost = OptimalLocalRouteCost;
			CopyTable(OptimalLocalRoute, OptimalRoute, this->Size);
			noImprove = 0;
			if(OptimalRouteCost < BestMinimumRouteCost)//Jezeli nowy coszt jest mniejszy od poprzedniego wiadomego poprzedniego
			 {
